...
app = Flask(__name__)
# Correct path: look for /data/menu.json inside project root
...
.header{background:#8B0000;color:white;padding:20px;text-align:center;border-radius:0 0 20px 20px}
.category-btn{font-size:20px;font-weight:bold;color:#8B0000;background:#fff}
.category-btn:not(.collapsed){background:#8B0000;color:white}
.accordion-button:focus{box-shadow:none}
.count-box{width:34px;height:34px;border-radius:50%;background:#8B0000;color:white;display:flex;align-items:center;justify-content:center;font-size:15px;font-weight:bold;flex-shrink:0}
...
.menu-switch{display:flex;gap:8px;overflow-x:auto;padding-bottom:5px}
.menu-switch a{flex:1;min-width:100px;text-align:center;text-decoration:none;border:1px solid #8B0000;border-radius:10px;padding:9px;color:#8B0000;background:white;font-weight:bold}
.menu-switch a.active{background:#8B0000;color:white}
</style></head><body>
...
{% for item in items %}
<div class="item"><div><b>{{item.item_name}}</b>
{% if item.category in ['Paneer Se Paneer Tak','Sabji Kuchh Hatke','Starters','Chinese','Soup','Sabiziyaan'] %}
<div class="spice-note">🌶 Spice: Normal • Medium • High</div>
{% endif %}
<div class="desc">{{item.description or ''}}</div></div><div class="price">₹{{item.price}}</div></div>
{% endfor %}
</div></div></div>
{% endfor %}
</div></div>
